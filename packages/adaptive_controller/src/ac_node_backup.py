...
class AdaptiveControllerNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        self.error_k = np.asarray([0.0, 0.0])
    ...
    def SaveCalibration(self):
        ...
        new_trim = - (np.asarray(self.theta_hat_k)*np.asarray(self.parameters['baseline'])*np.asarray(self.parameters['gain']))/(2.0*np.asarray(self.ref_k[0]))
